...
def initialize():
    ...
    data["actors"]={role:{"rmr":str(uuid4()),"piq":str(uuid4()),"email":role.lower()+"@example.invalid"} for role in ["CLIENT_ADMIN","SALES_REP","EXECUTIVE_VIEWER"]}
    data["actors"]["CLIENT_ADMIN"]["rmr"]=data["user"]
    data["actors"]["CLIENT_ADMIN"]["piq"]=data["piqUser"]
...
http {
  ...
  server {
    ...
    location / { proxy_pass http://rmr-federation-phase2-rmr:8000; proxy_set_header Host rmr.test; }
  }
  server {
    listen 443 ssl; server_name piq.test;
    ssl_certificate /proof/tls.crt; ssl_certificate_key /proof/tls.key;
    root /proof/piq-dist;
    location /api/ { proxy_pass http://rmr-federation-phase2-piq:4000; proxy_set_header Host piq.test; }
