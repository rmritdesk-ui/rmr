...
# v5.3.1 bounded correction assertions.
quick = text("rmr_platform/v531_final_corrections.py")
onboarding = text("rmr_platform/routes/onboarding.py")
portfolio = text("rmr_platform/routes/portfolio.py")
services = text("rmr_platform/services.py")
admin = text("public/pages/admin.js")
check("PC-01 Portfolio and Client Success KPIs are actionable", "data-portfolio-kpi" in admin and "Review Client" in admin and "data-success-kpi" in admin)
check("PC-02 Opportunity quick close is bounded to the existing Opportunity record", "quick-close" in quick and "Opportunity Closed Won" in quick and "Opportunity Closed Lost" in quick and "Mark Won" in frontend and "Mark Lost" in frontend)
check("PC-03 Onboarding validates objective stage requirements", "_validate_stage_requirements" in onboarding and "stage_requirements" in onboarding and "waiting_for_client_admin_activation" in onboarding)
check("PC-04 Invitation delivery status is honest", "Created / local delivery required" in portfolio and "Configured email delivery" in portfolio and "No email is claimed as sent" in portfolio)
check("PC-05 Solution activation creates operational follow-up", "solution_activation_follow_up" in services and "Confirm Activation" in admin and "Open Client 360" in admin)
check("PC-06 Forecast separates CRM and ProspectIQ provenance grids", "CRM Opportunity & Relationship Forecast" in frontend and "ProspectIQ Pipeline Potential" in frontend and "forecastAccountGrid" in frontend)
check("PC-07 CRM Leads presentation has dedicated layout styling", ".v53-lead-row" in css and "v53-row-actions" in frontend)
check("PC-08 Solutions cards contain long content and actions", "grid-template-rows" in css and "overflow-wrap:anywhere" in css and "v53-solution-card" in css)
check("PC-09 System Health uses progressive technical disclosure", "View Technical Details" in admin and "<details>" in admin)
check("Theme system remains excluded", "Tenant theme" not in frontend and "themes_in_scope" in text("control/V531-AUTHORIZED-CHANGESET.json"))
...
out = ROOT / "qa" / "V531-SCOPE-GATE.json"
